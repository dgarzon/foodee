class Recommendation < ActiveRecord::Base
  ...
  def self.get_friend_recommedation_by_restaurant(google_id, friend_id)
  	recommendation = Recommendation.where(user_id: User.select("id").where(id: friend_id),
  		restaurant_id: Restaurant.select("id").where(google_id: google_id))
    recommendation
  ...
  def self.get_my_recommedation_by_restaurant(user_id, google_id)
    recommendation = Recommendation.where(user_id: user_id, restaurant_id: Restaurant.select("id").where(google_id: google_id))
    recommendation
  end
